...
def enumAliasMembers(host, users):
    command = ["rpcclient", host, "-U", username + "%" + password, "-c"]
    aliases = enumAliases(host) # call enumAliases
    usrAliases = {} #dict of user Aliases
    
    for group in aliases:
        for name in users:
            users[name]["aliases"] = {group: {}}
            # Get a list of domain aliases for said user
...
def enumGroupMembers(host, users):
    command = ["rpcclient", host, "-U", username + "%" + password, + "-c"]
    groups = enumGroups(host) # Call enumGroups

    for name in users:
        users[name]["groups"] = {} # set users[name]["groups"] equal to an empty dict
...
def enumUsers(host):
    ...
    usersDict = enumGroupMembers(host, usersDict) # call enumGroupMembers
    usersDict = enumAliasMembers(host, usersDict) # call enumAliasMembers
